...
hours_sto = [900,1200]        # hours of power capacity that can be shifted using the storage capacity
share_sto  = [0,0.4]     # Storage Power divided by the peak load
share_wind = [0,0.3]     # Yearly wind generation divided by peak load * CF
share_pv = [0,0.3]       # Yearly PV generation divided by peak load *CF
r_ntc = [0,0.3]               # sum of the flow ntc divided by maximum load * weight
...
ref = {}
ref['overcapacity'] = ((units.PowerCapacity[flex_units].sum() + units.PowerCapacity[slow_units].sum() + units.PowerCapacity[sto_units].sum()) - load_max)/load_max
...
ref['rNTC'] = df_maxload['weigthed'].sum()     #mean?
...
for i in range(Nruns):
    ...
    net = r_ntc[0] + lh[i,6] * (r_ntc[-1] - r_ntc[0])

    folder = sim_folder + os.path.pardir + os.sep + "%.2f" % cap  + ' - ' + "%.2f" % flex + ' - ' + "%.2f" % hours + ' - ' + "%.2f" % sto + ' - ' + "%.2f" % wind + ' - ' + "%.2f" % pv + ' - ' + "%.2f" % net

    # in the first iteration, we load the input data from the original simulation directory:
    SimData = ds.adjust_capacity(sim_folder,('HPHS','WAT'),singleunit=True,value=load_max*sto)
    ...
    SimData = ds.adjust_storage(SimData,('BATS','OTH'),value=hours*load_max)
