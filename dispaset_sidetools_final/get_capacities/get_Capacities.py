...
ZONE = 'BE' #Zone in which we are doing the test           - TO IMPROVE
...
def define_PP():
    ...
    indexToDrop = PowerPlants[ (PowerPlants['Sort'].isna()) | (PowerPlants['Sort'] == 'HeatSlack') | (PowerPlants['Sort'] == 'THMS')].index
    print('[WARNING] : several tech are dropped as they are not referenced in the Dictionary : Sort. Here is the list : ',PowerPlants[PowerPlants['Sort'].isna()].index.tolist())
    print('[WARNING] : several tech are dropped as they become HeatSlack in DS. Here is the list : ',PowerPlants[PowerPlants['Sort'] == 'HeatSlack'].index.tolist())
    ...
    CHP_tech = list(PowerPlants.loc[PowerPlants['Sort']=='CHP'].index) #On cast le truc en list pour pouvoir le traiter facilement dans une boucle for
    P2HT_tech = list(PowerPlants.loc[PowerPlants['Sort']=='P2HT'].index) #On cast le truc en list pour pouvoir le traiter facilement dans une boucle for
    ELEC_tech = list(PowerPlants.loc[PowerPlants['Sort']=='ELEC'].index) #On cast le truc en list pour pouvoir le traiter facilement dans une boucle for
    STO_tech = list(PowerPlants.loc[PowerPlants['Sort']=='STO'].index) #On cast le truc en list pour pouvoir le traiter facilement dans une boucle for
    THMS_tech = list(PowerPlants.loc[PowerPlants['Sort']=='THMS'].index) #On cast le truc en list pour pouvoir le traiter facilement dans une boucle for
    ...
    for tech in ELEC_tech :        
        ...
        try :
            Efficiency = abs(layers_in_out.at[tech,'ELECTRICITY']/layers_in_out.at[tech,tech_ressource]) #If the TECH is ELEC , Efficiency is simply abs(ELECTRICITY/RESSOURCES)
        except :
    ...
    for tech in CHP_tech :
        ...
        try :
            ...
            Efficiency = abs(layers_in_out.at[tech,'ELECTRICITY']/layers_in_out.at[tech,tech_ressource]) #If the TECH is CHP  , Efficiency is simply abs(ELECTRICITY/RESSOURCES)
        except :
            print('[WARNING] : technology ',tech ,'has not been found in layers_in_out')
        ...
        PowerPlants.loc[PowerPlants.index==tech,'PowerCapacity'] = float(PowerPlants.at[tech,'PowerCapacity'])*PowerToHeatRatio
...
#    tot_sto_dhn_seasonal = sto_dhn_seasonal.sum()
    ...
    d = {'DHN_COGEN_GAS' : [0], 'DHN_COGEN_WOOD' : [0], 'DHN_COGEN_WET_BIOMASS' : [0], 'DHN_COGEN_WASTE' : [0], 'DHN_HP_ELEC' : [0]} 
    d2 = {'DHN_COGEN_GAS' : [0], 'DHN_COGEN_WOOD' : [0], 'DHN_COGEN_WET_BIOMASS' : [0], 'DHN_COGEN_WASTE' : [0], 'DHN_HP_ELEC' : [2.210936*10000000]} #10^7 
            ...
            if tech.startswith('DEC_') :
                ...
                if (sto_daily_cap < STO_THRESHOLD) & (sto_seasonal_cap < STO_THRESHOLD) : #case a
                    ...
                    if (sto_daily_cap > STO_THRESHOLD) & (sto_seasonal_cap > STO_THRESHOLD) : #case c
                        print('[ERROR] : case NOT HANDLED for the moment : TS_DHN_DAILY and TS_DHN_SEASONAL are implemented both above THMS Threshold in EnergyScope')
                        ...
                        if (sto_daily_cap > STO_THRESHOLD) :
                            ...
                            print('[INFO] : THMS',tech_sto_daily,'has a capacity installed of',sto_daily_cap,'split for',tech,'at the value of',float(STOCapacity))                        
...
#                            STOSelfDischarge = STO_caracteristics.at[tech_sto_daily,'storage_losses']
                            ...
                            print('[INFO] : THMS',tech_sto_seasonal,'has a capacity installed of',sto_seasonal_cap,'split for',tech,'at the value of',float(STOCapacity))                     
#                            print('[ERROR] : case NOT HANDLED for the moment : TS_DHN_DAILY and TS_DHN_SEASONAL are implemented above THMS Threshold in EnergyScope')
...
#            PowerPlants.at[tech,'STOCapacity'] = STOCapacity #in GWh
    ...
    for tech in STO_tech :
        ...
        try :
            STOCapacity = PowerPlants.at[tech,'PowerCapacity'] #GW to MW is done further #For other Tech ' f' in ES = PowerCapacity, whereas for STO_TECH ' f' = STOCapacity
            STOSelfDischarge = STO_caracteristics.at[tech,'storage_losses'] #In ES, the units are [%/s] whereas in DS the units are [%/h]
    
            #Caracteristics of charging and discharging 
            PowerCapacity = float(STOCapacity)/STO_caracteristics.at[tech,'storage_discharge_time'] # PowerCapacity = Discharging Capacity for STO_TECH in DS #GW to MW is done further
            STOMaxChargingPower = float(STOCapacity)/STO_caracteristics.at[tech,'storage_charge_time']*1000 #GW to MW
            ...
            PowerPlants.loc[PowerPlants.index==tech,['STOCapacity','STOSelfDischarge','PowerCapacity','STOMaxChargingPower','STOChargingEfficiency','Efficiency']] = [STOCapacity, STOSelfDischarge, PowerCapacity, STOMaxChargingPower, StoChargingEfficiency, Efficiency]
...
#    PowerPlants.fillna(0,inplace = True)
    ...
    Caracteristics = ['MinUpTime', 'MinDownTime', 'RampUpRate', 'RampDownRate', 'StartUpCost_pu', 'NoLoadCost_pu', 'RampingCost', 'PartLoadMin', 'MinEfficiency', 'StartUpTime', 'CO2Intensity','CHPPowerLossFactor']
    ...
    for index in index_list :
        ...
        if Typical_row.empty :
            print('[ERROR] : There was no correspondance for the Technology',Tech,'and fuel',Fuel,'in the Typical_Units file','(',index,')')
        ...
        #If the unit is present in Typical_Units.csv
            ...
            for carac in Caracteristics :        
                ...
                if len(value) == 0 :
                    print('[ERROR] : for caracteristics', carac,' no correspondance has been found for the Technology ',Tech,' and Fuel ',Fuel,'(',index,')')
                elif len(value) == 1 : #Normal case
                    value = float(value)
                    PowerPlants.loc[index,carac] = value
                elif len(value) > 1 :
                    print('[WARNING] : for caracteristics', carac,'size of value is > 1 for the Technology ',Tech,' and Fuel ',Fuel)
            ...
            if  (Typical_row['PowerCapacity'].values == 0.):
                ...
                Number_units = math.ceil(float(PowerPlants.loc[index,'PowerCapacity'])/float(Typical_row['PowerCapacity']))
                PowerPlants.loc[index,'Nunits'] = math.ceil(Number_units)
                PowerPlants.loc[index,'PowerCapacity'] = float(PowerPlants.loc[index,'PowerCapacity'])/math.ceil(Number_units)
    ...
    for index in index_CHP_list :
        ...
        Typical_row = Typical_Units.loc[(Typical_Units['Technology'] == Tech) & (Typical_Units['Fuel'] == Fuel)  & (Typical_Units['CHPType'] == CHPType) ]
                
        #If there is no correspondance in Typical_row
        if Typical_row.empty :
            print('[ERROR] : There was no correspondance for the COGEN',(Tech, Fuel, CHPType),'in the Typical_Units file','(',index,')')
        ...
        #If the unit is present in Typical_Units.csv
            ...
            for carac in Caracteristics :        
                ...
                #    3) > 1 : then .. What do we do ?
                if len(value) == 0 :
                    print('[ERROR] : for caracteristics', carac,' no correspondance has been found for the COGEN',(Tech, Fuel, CHPType),'in the Typical_Units file','(',index,')')
                elif len(value) == 1 : #Normal case
                    value = float(value)
                    PowerPlants.loc[index,carac] = value
                elif len(value) > 1 :
                ...
                if (carac == 'CHPPowerLossFactor') & (CHPType == 'back-pressure') :
                    if value > 0 :
                        print('[WARNING] the CHP back-pressure unit',Tech,'has been assigned a non-0 CHPPowerLossFactor. This value has to be forced to 0 to work with DISPA-SET')
            ...
            if  not (float(PowerPlants.loc[index,'STOCapacity']) > 0) : #If there is a Thermal Storage and then a STOCapacity at the index
