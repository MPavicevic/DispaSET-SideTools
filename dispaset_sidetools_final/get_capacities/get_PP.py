...
input_folder2 = '../../Inputs/' #to access Assets.txt
...
def search_assets(tech, feat):
    ...
    assets = pd.read_csv(input_folder2 + 'Assets.txt', delimiter='\t')
...
def define_PP():
    ...
    #         9) StartUpCost 
    #         10) RampingCost
    Typical_Units = pd.read_csv(input_folder + 'Typical_Units_modif.csv')
...
#                            print('[INFO] : THMS',tech_sto_daily,'has a capacity installed of',sto_daily_cap)
                            ...
                            #To set the STOCapacity, it is not so trivial - TO DO
                
                            STOSelfDischarge = STO_caracteristics.at[tech_sto_daily,'storage_losses']
                            PowerPlants.at[tech,'STOSelfDischarge'] = STOSelfDischarge #in GWh 
                            
                        elif (sto_seasonal_cap > STO_THRESHOLD) :
#                            print('[INFO] : for tech',tech,'THMS',tech_sto_seasonal,'has a capacity installed of',sto_seasonal_cap)
...
#            PowerPlants.at[tech,'STOCapacity'] = STOCapacity #in GWh
    ...
    for tech in STO_tech :
        ...
        try :
            STOCapacity = PowerPlants.at[tech,'PowerCapacity'] #For other Tech ' f' in ES = PowerCapacity, whereas for STO_TECH ' f' = STOCapacity
            STOSelfDischarge = STO_caracteristics.at[tech,'storage_losses'] #*3600 #In ES, the units are [%/s] whereas in DS the units are [%/h]
    
            #Caracteristics of charging and discharging 
            PowerCapacity = float(STOCapacity)/STO_caracteristics.at[tech,'storage_discharge_time'] # PowerCapacity = Discharging Capacity for STO_TECH in DS
            STOMaxChargingPower = float(STOCapacity)/STO_caracteristics.at[tech,'storage_charge_time']*1000
...
#    PowerPlants.fillna(0,inplace = True)
    ...
    for index in index_list :
        ...
        if Typical_row.empty :
            print('[ERROR] : There was no correspondance for the Technology',Tech,'and fuel', Fuel,'in the Typical_Units file')
        ...
        #If the unit is present in Typical_Units.csv
            ...
            for carac in Caracteristics :        
                ...
                if len(value) == 0 :
                    print('[ERROR] : for caracteristics', carac,' no correspondance has been found for the Technology ',Tech,' and Fuel ',Fuel)
