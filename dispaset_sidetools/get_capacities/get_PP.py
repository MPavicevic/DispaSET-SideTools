            ...
            if tech.startswith('DEC_'):
                ...
                if (sto_daily_cap < STO_THRESHOLD) & (sto_seasonal_cap < STO_THRESHOLD):  # case a
                    ...
                    if (sto_daily_cap > STO_THRESHOLD) & (sto_seasonal_cap > STO_THRESHOLD):  # case c
                        print(
                            '[ERROR] : case NOT HANDLED for the moment : TS_DHN_DAILY and TS_DHN_SEASONAL are implemented above THMS Threshold in EnergyScope')
                        ...
                        if (sto_daily_cap > STO_THRESHOLD):
                            ...
                            STOSelfDischarge = STO_caracteristics.at[
                                tech_sto_seasonal, 'storage_losses']  # TS_DHN_SEASONAL
                            PowerPlants.at[tech, 'STOSelfDischarge'] = STOSelfDischarge  # in GWh
    ...
    for tech in STO_tech:
        ...
        try:
            STOCapacity = PowerPlants.at[
                tech, 'PowerCapacity']  # For other Tech ' f' in ES = PowerCapacity, whereas for STO_TECH ' f' = STOCapacity
            STOSelfDischarge = STO_caracteristics.at[
                tech, 'storage_losses']  # *3600 #In ES, the units are [%/s] whereas in DS the units are [%/h]

            # Caracteristics of charging and discharging
            PowerCapacity = float(STOCapacity) / STO_caracteristics.at[
                tech, 'storage_discharge_time']  # PowerCapacity = Discharging Capacity for STO_TECH in DS
            ...
            for carac in Caracteristics:
                ...
                if len(value) == 0:
                    print('[ERROR] : for caracteristics', carac,
                          ' no correspondance has been found for the Technology ', Tech, ' and Fuel ', Fuel)
