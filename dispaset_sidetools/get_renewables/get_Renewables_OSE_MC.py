...
data_PHOT = pd.read_excel(input_folder + 'Default/' + 'res_time_series_8760h.xlsx','pv',index_col = 0)
data_PHOT.rename(columns={'GB' : 'UK', 'GR' : 'EL'},inplace=True)
data_WTON = pd.read_excel(input_folder + 'Default/' + 'res_time_series_8760h.xlsx','wind onshore',index_col = 0)
data_WTON.rename(columns={'GB' : 'UK', 'GR' : 'EL'},inplace=True)
data_WTOF = pd.read_excel(input_folder + 'Default/' + 'res_time_series_8760h.xlsx','wind offshore',index_col = 0)
data_WTOF.rename(columns={'GB' : 'UK', 'GR' : 'EL'},inplace=True)
data_WTON_WTOF = pd.read_excel(input_folder + 'Default/' + 'res_time_series_8760h.xlsx','wind on&off combined',index_col = 0)
...
for region in regions:
    ...
    # Hydro run-of-river
    tmp_HROR = pd.read_csv(input_folder +'Default/' + 'AvailabilityFactors/' + region + '/1h/2016.csv')
    ...
    try:
        tmp_InFlows = pd.read_csv(input_folder + 'Default/' + 'HydroData/ScaledInflows/' + region + '/1h/2016_profile_from_2012.csv')
        ...
        tmp_InFlows = pd.read_csv(input_folder + 'Default/' + 'HydroData/ScaledInflows/' + 'AT' + '/1h/2016_profile_from_2012.csv')
        tmp_InFlows = tmp_InFlows.head(8760)
        tmp_HDAM = pd.DataFrame(tmp_InFlows['HDAM'].values, index=drange, columns = ['HDAM'])        
        tmp_HPHS = pd.DataFrame(tmp_InFlows['HPHS'].values, index=drange, columns = ['HPHS'])
        tmp_inflow_timeseries = [tmp_HDAM,tmp_HPHS]
    ...
    try:    
        tmp_Reservoirs = pd.read_csv(input_folder + 'Default/' + 'HydroData/ScaledLevels/' + region + '/1h/2016.csv')
        ...
        tmp_Reservoirs = pd.read_csv(input_folder+ 'Default/' + 'HydroData/ScaledLevels/' + 'AT' + '/1h/2016.csv')
        tmp_Reservoirs = tmp_Reservoirs.head(8760)
...
def write_csv_files(file_name_AF,file_name_IF,file_name_RL,res_timeseries,inflow_timeseries,reservoirs_timeseries, scenario, write_csv=None):
    '''
    ...
    if write_csv == True:
        ...
        for c in inflow_timeseries:     
            make_dir(output_folder + source_folder + 'Database' + scenario +'/')
            make_dir(output_folder + source_folder + 'Database/' + scenario + '/HydroData/')
...
write_csv_files('OSE_AvailabilityFactors_2030','OSE_IF_2030','OSE_RL_2030',res_timeseries,inflow_timeseries,reservoirs_timeseries,'ProRes1', True)
write_csv_files('OSE_AvailabilityFactors_2030','OSE_IF_2030','OSE_RL_2030',res_timeseries,inflow_timeseries,reservoirs_timeseries,'NearZeroCarbon', True)
