...
from dispaset_sidetools.common import date_range, get_country_codes, write_csv_files, commons, make_dir, \
    make_timeseries, clean_convert, reshape_timeseries
...
with open(input_folder + source_folder + temba_folder + 'TEMBA_results.p', 'rb') as handle:
...
def rename_scenarios(data, string):
    data = {k.replace(string, ''): v for k, v in data.items()}
    return data

to_rename = 'TEMBA_'
inputs = rename_scenarios(inputs, to_rename)
results = rename_scenarios(results, to_rename)
r = rename_scenarios(r, to_rename)
costs = rename_scenarios(costs, to_rename)
operation = rename_scenarios(operation, to_rename)
...
# %% Curtailment
def get_stats(inputs, results, variable, zones):
    scenarios = list(results.keys())
    ...
    for s in scenarios:
        ...
        tmp.loc['Median', s] = results[s][variable].loc[:, results[s][variable].columns.isin(zones)].sum(axis=1).median()
        tmp.loc['Hours', s] = (results[s][variable].loc[:, results[s][variable].columns.isin(zones)].sum(axis=1) != 0).sum()
    return tmp

all_pp = [power_pools[0], power_pools[1], power_pools[2],['BI', 'DJ', 'EG', 'ET', 'ER', 'KE', 'RW', 'SO', 'SD', 'SS', 'TZ', 'UG',
               'DZ', 'LY', 'MA', 'MR', 'TN','AO', 'CM', 'CF', 'CD', 'CG', 'GA', 'GQ', 'TD']]
i = 0
for z in all_pp:
    if i == 0:
    ...
    curtailment = get_stats(inputs, results, 'OutputCurtailedPower', z)
    shed_load = get_stats(inputs, results, 'OutputShedLoad', z)
    ...
    zones = inputs[scenarios[0]]['sets']['n']
    ...
    ax2 = curtailment.loc['Percentage peak VRES', :].T.plot(kind='bar', color='firebrick', secondary_y=True, rot=0, ax=ax,
                                                            width=0.3, position=0, mark_right=False,
                                                            fontsize=15)
...
ax.set_xlabel('Scenario', fontsize=15)
ax.set_title("$CO_{2}$ emissions - High NTC", fontsize=15)
    ...
    for s in scenarios:
        for f in commons['Fuels']:
            ...
            generation_breakdown.loc[idx, s] = results[s]['OutputPower'].T.loc[(filter_by_zone(inputs[s], zones)) &
                                                (filter_by_fuel(inputs[s], [f]))].sum().sum()
...
for z in power_pools:
    ...
    plot_fuel_bar(get_water_indicators(r, z)[1], y_label='Consumption (billion m3)', x_label='Scenario',
                  title='Water Consumption ' + png_name, sci_limits=(0, 0), png_name='Water Consumption ' + png_name)
    plot_fuel_bar(get_generation_indicators(results, z), y_label='Generation [TWh]', x_label='Scenario',
                  title='Total Generation ' + png_name, sci_limits=(0, 0), png_name='Generation Breakdown ' + png_name)
    i = i +1


# %% Plot heatmap
def plot_heatmap(Load, x='dayofyear', y='hour', aggfunc='sum', bins=8, figsize=(15,14), edgecolors='none',
                 cmap='Oranges', colorbar=True, columns = 1, x_ax_label = [0,2], y_ax_label = [0,2], png_name = None,
                 **pltargs):
...
# plot_heatmap(results['1999']['ShadowPrice'], bins=30, figsize=(10,9), cmap='RdBu_r', colorbar=True, columns = 5,
#              x_ax_label = [20,21,22,23,24], y_ax_label = [0,5,10,15,20], png_name='Shadow Price Average')

plot_heatmap(results['1985']['StorageShadowPrice'], bins=30, figsize=(20,9), cmap='RdBu_r', colorbar=True, columns = 3,
             x_ax_label = [10,11, 12], y_ax_label = [0,3,6,9,12], png_name='Storage Shadow Price Wet')
plot_heatmap(results['2009']['StorageShadowPrice'], bins=30, figsize=(20,9), cmap='RdBu_r', colorbar=True, columns = 3,
...
for z in power_pools:
    ...
    plot_fuel_bar(startup, y_label='Startups [-]', x_label='Scenario', title='Total Startups ' + png_name,
                  sci_limits=(0, 0), png_name='Startups ' + png_name)
    shutdown = get_startup_indicators(aa[1], z)
    plot_fuel_bar(shutdown, y_label='Shutdowns [-]', x_label='Scenario', title='Total Shutdowns ' + png_name,
...
inflows_KENITRA = pd.read_csv(input_folder + source_folder + 'AfricaDamsInFlows/' + '067_Allal_Al_Fassi.csv', index_col = 0, parse_dates = ['time'])
...
inflows_GQ = pd.read_csv(input_folder + source_folder + 'AfricaDamsInFlows/' + '039_Djiboloho.csv', index_col = 0, parse_dates = ['time'])
...
inflows_RW = pd.read_csv(input_folder + source_folder + 'AfricaDamsInFlows/' + '083_Ntaruka.csv', index_col = 0, parse_dates = ['time'])
